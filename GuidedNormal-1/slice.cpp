...
void Slice::GetContour(TriMesh &mesh, std::vector<int> &edges_status, TriMesh::HalfedgeHandle first_heh, double &height, Polylines &contour)//get contour from a intersected halfedge.
{
...
void Slice::SliceTheModel(TriMesh &mesh)
{
	...
	for (int i = 1; i < (layers_number+1); i++)//get intersected contours layer by layer
	{
		...
		Contours z_contours;//intersected contours of each layer
